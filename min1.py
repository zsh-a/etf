...
origin = pd.read_csv(
    "etf/min1/hfq/510880.csv", index_col="datetime", parse_dates=["datetime"]
)
# stock_hfq_df.to_csv('t.csv',index=False)
stock_hfq_df = origin.resample('2min').agg({'open': 'first', 'high': 'max', 'low': 'min', 'close': 'last', 'volume': 'sum', 'amount': 'sum'}).dropna()
# stock_hfq_df = origin
...
class TestStrategy3(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.pos:
            if self.data.datetime.time() == datetime.time(9, 32):  # 检查是否是第一个5分钟线
                # print(self.dataclose[0] ,self.dataclose[-1])
                if self.dataclose[0] > self.dataopen[0]:
                    cash = self.broker.get_cash()
                    size = int(cash * 0.99 / self.data.close[0] / 100) * 100  # 计算最大可用资金买入的股数
            ...
            if self.count >= self.params.period and self.data.datetime.time() == datetime.time(14, 58):  # 检查是否是收盘时间
                pos = self.getposition()
                self.sell(size=pos.size)
                self.pos = False
                self.count = 0
...
start_date = datetime.datetime(2022, 12, 16)  # 回测开始时间
...
sh_csv = pd.read_csv(
    "etf/min1/hfq/999999.csv", index_col="datetime", parse_dates=["datetime"]
)
